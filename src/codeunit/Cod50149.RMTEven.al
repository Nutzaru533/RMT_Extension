codeunit 50149 "RMT Even"
{
    ...
    [EventSubscriber(ObjectType::Table, Database::"Tax Report Line", 'OnBeforeInsertVatLine', '', true, true)]
    local procedure "Tax Report Line OnBeforeInsertVatLine"
    (
        var TaxReportLine: Record "Tax Report Line"
    //var VatTransection: Record "VAT Transections"
    )
    begin
        if TaxReportLine."Tax Invoice Date" = 0D then begin
            TaxReportLine."Tax Invoice Date" := TaxReportLine."Posting Date";
        end;
    end;
