page 50101 "RMT Purchase Requests"
{
  ...
  SourceTableView = WHERE("Document Type"=CONST(Quote), "RMT Purchase Request"=filter(true));
  UsageCategory = Lists;
    ...
    area(processing)
    {
      ...
      group(Action12)
      {
        ...
        action(Print)
        {
          ...
          trigger OnAction()var ApplicationAreaMgmtFacade: Codeunit "Application Area Mgmt. Facade";
      ...
      group(Action3)
      {
        ...
        action(UpdatePurchaseRequest)
        {
          ...
          trigger OnAction()var PurchaesH: Record "Purchase Header";
          purchaseL: Record "Purchase Line";
          begin
            PurchaesH.reset;
            PurchaesH.SetRange("Document Type", PurchaesH."Document Type"::Quote);
            PurchaesH.SetRange("RMT Purchase Request", true);
            if PurchaesH.Find('-')then begin
              repeat purchaseL.reset;
                purchaseL.SetRange("Document No.", PurchaesH."No.");
                purchaseL.SetRange("Document Type", purchaseL."Document Type"::Quote);
                if purchaseL.Find('-')then begin
                  repeat purchaseL."RMT Purchase Request":=true;
                    purchaseL.Modify();
                  until purchaseL.next = 0;
                end;
              until PurchaesH.Next() = 0;
            end;
          end;
        }
      }
