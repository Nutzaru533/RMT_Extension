tableextension 50107 "RMT Tax Report" extends "Tax Report Line"
{
    fields
    {
        modify("Document No.")
        {
            ...
            var
                myInt: Integer;
            begin
                if "Tax Invoice No." = '' then begin
                    "Tax Invoice No." := "Document No.";

                end;
                if "Tax Invoice Date" = 0D then begin
                    "Tax Invoice Date" := "Posting Date";
                end
            end;
        }
    ...
    begin
        ...
        if VatTransection.FindSet() then
            repeat
                if not varPostingSsetup.GET(VatTransection."VAT Bus. Posting Group", VatTransection."VAT Prod. Posting Group") then varPostingSsetup.init;
                var_Skip := false;
                checkrevers := false;
